...
module ThinClient
	class Input
		# input text
		def self.text(str, device = "onlyone")

			#tmp = `adb shell input text`
			tmp = Myadb.shell(device, "input text")
			...
			begin
			  ...
			  Myadb.shell(device, "input text #{strings[0]}")
			  if (strings.size > 1)
			  	strings[1..-1].each{ |string|
			  		#system("adb shell input keyevent KEYCODE_SPACE")
			  		#system("adb shell input text #{string}")
			  		Myadb.shell(device, "input keyevent KEYCODE_SPACE")
			  		Myadb.shell(device, "input text #{string}")
			  	}
			  end
			rescue => ex
			  Log.error("#{ex}")
		...
		# input tap
		def self.tap(x = 0, y = 0, device= "onlyone")
			...
			Myadb.shell(device, "input tap #{x} #{y}")
			return true
		end

		# input keyevent
		def self.keyevent(keycode, device = "onlyone")

			#tmp = `adb shell input keyevent`
			Myadb.shell(device, "input keyevent")
			...
			tmp2 = Myadb.shell(device, "input keyevent #{keycode}")
		...
		# input swipe
		def self.swipe(x1 = 0, y1 = 0, x2 = 0, y2 = 0, device = "onlyone")
			#tmp = `adb shell input swipe`
			tmp = Myadb.shell(device, "input swipe")
			...
			Myadb.shell(device, "input swipe #{x1} #{y1} #{x2} #{y2}")
			return true
		end
	end
end